"""The emulator must serve the demo project over HTTP, the way real GCP would."""

from __future__ import annotations


def test_serves_buckets(provider):
    buckets = provider.list_buckets()
    assert any(b["name"] == "customer-data-backup" for b in buckets)


def test_serves_iam_policy(provider):
...
def test_serves_service_account_keys(provider):
    keys = provider.list_service_account_keys()
    assert any(k["keyType"] == "USER_MANAGED" for k in keys)


def test_serves_firewall_rules(provider):
    rules = provider.list_firewall_rules()
    assert any(r["name"] == "allow-all-ingress" for r in rules)


def test_serves_cloud_run_services(provider):
